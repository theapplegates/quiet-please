...
for (const imageRef of imageRefs) {
  ...
  const width = result.width || "TODO";
  const height = result.height || "TODO";
  const breakpointStr = widths.join(", ");

  console.log(`Successfully handled: ${publicId}`);
  console.log(`Breakpoints: ${breakpointStr}`);
